	...
	editText.keydown(function(e) {
		if (e.which == 13) { // Enter
			if (editText.val() != value && editText.val().trim().length > 0) {
				 if((editText.val()>0 && editText.val()<=5)){
				...
				editText.remove();
					 ...
					 showToster('Error !',"Oops ! Rating should not be greater then 5", 5, "error");
				 }
		} else if (e.which == 27) {
			editText.unbind("focusout");
			editText.remove();
...
function initializeEmployeeDropdown(){
	...
	$('#empDropdown').multiselect({
       ...
       /* onChange: function (element, checked) {
        	var items = $('#empDropdown_'+empId+ 'option:selected').val();
            }
   */ });	
}
...
function sendMailToSpecificEmp(){
	...
	//var dataStringForMail = "empId="+empIdForMail+"&subject="+subjectForMail+"&configMailtext="+configMailtext+"";*/
	   $.ajax({
	       type : "POST",
...
/*function sendMailToSpecificEmp(){
	debugger
	   $('#configMailtext').html( $('.configMailSummerNote').code()); //save HTML If you need(aHTML: array).
	   $.ajax({
	       type : "POST",
	       url : sendEmailToSingleEmpURL,
...
function cycleDatesValidations() {
	...
	$('#selfApprStartDate').datepicker({
		...
		$('#selfApprStartDate').datepicker().on('changeDate', function() {
			...
			$('#selfApprEndDate').datepicker().on('changeDate', function() {
				var temp1 = $(this).datepicker('getDate');
				var d1 = new Date(temp1);
				...
				$('#mngApprStartDate').datepicker().on('changeDate', function() {
					...
					$('#mngApprEndDate').datepicker().on('changeDate', function() {
						var temp1 = $(this).datepicker('getDate');
						var d1 = new Date(temp1);
						d1.setDate(d1.getDate());
					$('#revApprStartDate').datepicker({
					...
					$('#revApprStartDate').datepicker().on('changeDate', function() {
						var temp1 = $(this).datepicker('getDate');
						var d5 = new Date(temp1);
...
function ClearDates() {
	$('#cycleStartDate').data('datepicker').setDate(null);
	$('#cycleEndDate').data('datepicker').setDate(null);
	$('#selfApprStartDate').data('datepicker').setDate(null);
	$('#selfApprEndDate').data('datepicker').setDate(null);
	$('#mngApprStartDate').data('datepicker').setDate(null);
	$('#mngApprEndDate').data('datepicker').setDate(null);
	$('#revApprStartDate').data('datepicker').setDate(null);
	$('#revApprEndDate').data('datepicker').setDate(null);
	}
